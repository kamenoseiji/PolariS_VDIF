	...
	unsigned int	*bitDist)	// Bit distribution counter	(8 IF x 4 levels)
{
	...
	for(index=0; index<nbytes; index+=4){			// 4 bytes per sample
		...
		bitDist[2* nlevel + ((data_ptr[index  ] >> 4) & bitmask)] ++;	// IF-2 bitdist
		bitDist[3* nlevel + ((data_ptr[index  ] >> 6) & bitmask)] ++;	// IF-3 bitdist
		bitDist[4* nlevel + ((data_ptr[index+1]     ) & bitmask)] ++;	// IF-4 bitdist
		bitDist[5* nlevel + ((data_ptr[index+1] >> 2) & bitmask)] ++;	// IF-5 bitdist
		bitDist[6* nlevel + ((data_ptr[index+1] >> 4) & bitmask)] ++;	// IF-6 bitdist
		bitDist[7* nlevel + ((data_ptr[index+1] >> 6) & bitmask)] ++;	// IF-7 bitdist
	}
	return(nbytes);
}
	...
	unsigned int	*bitDist)	// Bit distribution counter	(16 IF x 4 levels)
{
	...
	for(index=0; index<nbytes; index+=4){			// 4 bytes per sample
		...
		bitDist[ 6* nlevel + ((data_ptr[index+1] >> 4) & bitmask)] ++;	// IF-6 bitdist
		bitDist[ 7* nlevel + ((data_ptr[index+1] >> 6) & bitmask)] ++;	// IF-7 bitdist
		bitDist[ 8* nlevel + ((data_ptr[index+2]     ) & bitmask)] ++;	// IF-8 bitdist
		bitDist[ 9* nlevel + ((data_ptr[index+2] >> 2) & bitmask)] ++;	// IF-9 bitdist
		bitDist[10* nlevel + ((data_ptr[index+2] >> 4) & bitmask)] ++;	// IF-10 bitdist
		bitDist[11* nlevel + ((data_ptr[index+2] >> 6) & bitmask)] ++;	// IF-11 bitdist
		bitDist[12* nlevel + ((data_ptr[index+3]     ) & bitmask)] ++;	// IF-12 bitdist
		bitDist[13* nlevel + ((data_ptr[index+3] >> 2) & bitmask)] ++;	// IF-13 bitdist
		bitDist[14* nlevel + ((data_ptr[index+3] >> 4) & bitmask)] ++;	// IF-14 bitdist
		bitDist[15* nlevel + ((data_ptr[index+3] >> 6) & bitmask)] ++;	// IF-15 bitdist
	}
	return(nbytes);
}
